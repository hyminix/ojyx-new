// --- Models/PlayerBoard.cs --- (Ajout des méthodes de vérification et de suppression)
...
        // AJOUT : Vérifie si une colonne est complète (3 cartes identiques face visible).
...
        public List<(int row, int col)> CheckForCompletedColumns()

        {
...

            {
                ...
                if (card1 != null && card2 != null && card3 != null && // Vérifie que les cartes existent.

                            card1.IsFaceUp && card2.IsFaceUp && card3.IsFaceUp && // Vérifie qu'elles sont face visible.

                            card1.Data.value == card2.Data.value && card2.Data.value == card3.Data.value) // Vérifie qu'elles sont identiques.
...
        // AJOUT : Vérifie si une ligne est complète (4 cartes identiques face visible).
...
        // AJOUT : Supprime les cartes aux positions données et retourne les cartes supprimées.

        public List<Card> RemoveCardsAt(List<(int row, int col)> positions)

        {
...

            {
                ...
                Card cardToRemove = GetCardAt(pos.row, pos.col);

                if (cardToRemove != null)

                {

                    //Trouver le slot et le supprimer

                    foreach (var slot in cardSlots)

                    {

                        if (slot.row == pos.row && slot.column == pos.col)
